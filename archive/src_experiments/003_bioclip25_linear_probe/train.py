"""
Supervised linear-probe training: frozen BioCLIP 2.5 backbone + linear head.
...
--------------
  ...
  Cached embeddings (fastest - run build_cache.py first):
      python train.py --use-cache --cache-dir ./cache [args]

Key design choices
------------------
  - Backbone always frozen; only the linear head trains.
  - DistributedDataParallel wraps the full model when running multi-GPU.
  - Only rank-0 writes logs, checkpoints, and JSON output.
  - Mixed precision (AMP) enabled by default (--no-amp to disable).
  - Validation runs on rank-0 only for simplicity; loss is reduced across ranks.
  - Training loss is all-reduced so the logged value is the true average.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="BioCLIP 2.5 linear probe training for PlantCLEF.",
    ...
    p.add_argument("--cache-dir", default="./cache",
                   help="Directory containing embeddings.pt, labels.pt from build_cache.py.")
...
def validate(
    ...
    """
    Evaluate on the val split and return metrics dict.

    Works in both normal mode (images → backbone → head) and cache mode
    (embeddings → head directly).
    """
    model.eval()
    results: list[dict] = []
    total_loss = 0.0
...
def main() -> None:
    ...
    if is_main_process():
        logger.info("=" * 60)
        logger.info("BioCLIP 2.5 Linear Probe Training")
    ...
    # ------------------------------------------------------------------
    if args.use_cache:
        # Cache mode: pre-computed embeddings
        cache_dir = Path(args.cache_dir)
        train_embeddings_path = str(cache_dir / "train_embeddings.pt")
        train_labels_path     = str(cache_dir / "train_labels.pt")
        val_embeddings_path   = str(cache_dir / "val_embeddings.pt")
        val_labels_path       = str(cache_dir / "val_labels.pt")
        class_map_path        = str(cache_dir / "class_mapping.txt")

        from dataset import load_class_mapping
        species_to_idx, idx_to_species = load_class_mapping(class_map_path)
        ...
        # Normal mode: load images, run backbone live
        if is_main_process():
    ...
    # Val loader: only used on rank 0, no DDP sharding needed
    ...
    # ------------------------------------------------------------------
    if args.use_cache:
        # Cache mode: backbone not needed - train a standalone linear head.
        embed_dim = train_dataset[0][0].shape[0]
        if is_main_process():
            logger.info(f"Cache mode: embed_dim={embed_dim}, training head-only")
        # Wrap in a tiny module so DDP, optimizer, and checkpoint code stay uniform.
    ...
    if args.resume:
        ...
        if is_main_process():
            logger.info(f"Resuming from epoch {start_epoch}  best_r@5={best_recall_at_5:.4f}")
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # Reduce loss across ranks for accurate reporting
        ...
        if is_main_process():
            ...
            # Cache mode saves only head weights; full mode saves entire model.
            if args.use_cache:
                ckpt_state = {
                    "epoch":              epoch,
