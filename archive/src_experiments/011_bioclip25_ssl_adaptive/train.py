...
SSL warm-start
--------------
  --ssl-backbone-checkpoint path/to/backbone.pt
      Load backbone weights from an SSL pre-training checkpoint before
      supervised training begins.  Uses strict=False; missing/unexpected
...
from model import BioCLIP25MultiTask, BIOCLIP25_MODEL_NAME, load_ssl_backbone
...
def parse_args() -> argparse.Namespace:
    ...
    # SSL backbone warm-start
    p.add_argument(
        "--ssl-backbone-checkpoint",
        default=None,
        metavar="PATH",
        help="Path to backbone.pt from train_ssl.py. Loaded with strict=False before training.",
    )
...
def main() -> None:
    ...
    if is_main_process():
        ...
        logger.info(f"  ssl_backbone  : {args.ssl_backbone_checkpoint or 'none'}")
    ...
    # Load SSL backbone weights before moving to device / DDP wrap
    if args.ssl_backbone_checkpoint:
        if is_main_process():
            load_ssl_backbone(model, args.ssl_backbone_checkpoint)
        barrier()
