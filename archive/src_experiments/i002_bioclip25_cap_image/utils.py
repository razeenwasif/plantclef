...
def compute_multitask_loss(
    ...
    """
    Compute weighted multi-task classification loss (species + genus + family).

    outputs : (sp_logits, gen_logits, fam_logits)
              Taxonomy logits can be None when that head is disabled.
    labels  : (sp_labels, gen_labels, fam_labels)
              Taxonomy labels == -1 for samples missing that annotation.

    Returns (total_loss, loss_components_dict).
    """
    sp_log, gen_log, fam_log = outputs
    sp_lbl, gen_lbl, fam_lbl = labels
