...
fn generate_index(tar_path: &Path, idx_path: &Path) -> Result<()> {
    ...
    loop {
        ...
        // Extract filename (first 100 bytes of TAR header)
    ...
    for s in samples {
        // v1.2 Official C++ Format: <ext> <offset> <size> <filename>
        out.push_str(&format!(
            "jpg {} {} {} cls {} {} {}\n", 
...
fn main() -> Result<()> {
    ...
    if !shard_dir.exists() {
        println!("[Error] Shard directory not found.");
        return Ok(());
    }
    ...
    let mut tar_files: Vec<PathBuf> = std::fs::read_dir(shard_dir)?
        ...
        .filter(|p| {
            ...
            name.starts_with("train_") && p.extension().and_then(|s| s.to_str()) == Some("tar")
        })
        .collect();
    tar_files.sort();

    println!("[*] Found {} training shards. Generating v1.2 DALI indices...", tar_files.len());

    
    let pb = ProgressBar::new(tar_files.len() as u64);
    pb.set_style(ProgressStyle::with_template("[{elapsed_precise}] [{bar:40.cyan/blue}] {pos}/{len} Shards (ETA: {eta})").unwrap());
    ...
    println!("[+] SUCCESS: Correct v1.2 indices ready. Instant launch achieved!");
    Ok(())
}
