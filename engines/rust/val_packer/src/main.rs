...
fn main() -> Result<()> {
    ...
    println!("[*] val_packer: Sharding only the 5% Validation set...");
    ...
    let val_records = &records[0..val_size];
    
    println!("[*] Found {} validation images. Packing into {} shards...", val_records.len(), (val_records.len() + SHARD_SIZE - 1) / SHARD_SIZE);

    rayon::ThreadPoolBuilder::new().num_threads(MAX_THREADS).build_global().unwrap_or(());

    let num_shards = (val_records.len() + SHARD_SIZE - 1) / SHARD_SIZE;
    let pb = ProgressBar::new(num_shards as u64);
    pb.set_style(ProgressStyle::default_bar().template("[{elapsed_precise}] [{bar:40.cyan/blue}] {pos}/{len} Shards")?);

    (0..num_shards).into_par_iter().for_each(|shard_idx| {
        let shard_path = Path::new(OUTPUT_DIR).join(format!("val_{:05}.tar", shard_idx));
        let _ = stream_shard(shard_idx, val_records, &shard_path);
        pb.inc(1);
    });

    println!("[+] SUCCESS: Validation shards ready.");
    Ok(())
}
...
fn stream_shard(shard_idx: usize, records: &[Record], shard_path: &Path) -> Result<()> {
    ...
    for i in start..end {
        ...
        if let Ok(img) = image::open(&img_path) {
            ...
            if let Ok(src_image) = fr::Image::from_vec_u8(NonZeroU32::new(width).unwrap(), NonZeroU32::new(height).unwrap(), img.to_rgb8().into_raw(), fr::PixelType::U8x3) {
                let mut dst_image = fr::Image::new(NonZeroU32::new(TARGET_SIZE).unwrap(), NonZeroU32::new(TARGET_SIZE).unwrap(), fr::PixelType::U8x3);
