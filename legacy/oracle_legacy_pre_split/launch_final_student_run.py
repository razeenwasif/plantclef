...
def main() -> None:
    ...
    if args.multi_node:
        ...
        # plantclef: Configuration for the 4-GPU cluster
        ...
        for rank in [0, 1]:
            ...
            print(f"[Launcher] PLANTCLEF: Started Local Rank {rank} on GPU {rank}")
        ...
        for rank in [2, 3]:
            ...
            processes.append((p, log_f))
            print(f"[Launcher] PLANTCLEF: Started Remote Rank {rank} on Pod B GPU {local_r}")
    ...
    if args.tpu:
        ...
        print(f"[Launcher] Targeting NVIDIA CUDA ({args.num_gpus} GPUs - PLANTCLEF Isolated Mode)")
        os.environ["PLANTCLEF_HARDWARE"] = "CUDA"
        
        # plantclef: Absolute Process-Level VRAM Isolation
        # Launching separate processes with unique CUDA_VISIBLE_DEVICES
        processes = []
        import time
        for rank in range(args.num_gpus):
            # plantclef: Hard-Staggered Launch (30s head start for Rank 0)
            # This allows the master to finish the Disk Scan and create 
            # the integrity cache before Rank 1 tries to read it.
            if rank > 0:
                print(f"[Launcher] PLANTCLEF: Rank 0 head-start... Rank {rank} waiting 30s.")
                time.sleep(30) 
            ...
            # plantclef: Inherit world size and offset from terminal if present
            ...
            env["PYTHONUNBUFFERED"] = "1" # plantclef: Force immediate log flushing
            ...
            # plantclef: Force unbuffered binary stream for real-time logging
            ...
            print(f"[Launcher] PLANTCLEF: Started Rank {rank} (Unbuffered Log: models/cuda_deep_sat/rank_{rank}.log)")
            
        try:
            print(f"[Launcher] PLANTCLEF: Cluster monitoring active. Watch logs for progress.")
