...
class OracleOrchestrator:
    def __init__(self, phases, role=None):
        ...
        print(f"🛡️  PLANTCLEF: Role={self.role}, LocalIP={self.local_ip}, MasterIP={self.master_ip}")
    ...
    def run_master(self):
        ...
        # plantclef: Enable TCP KeepAlives on the server socket
        ...
        while len(workers) < TOTAL_WORKERS:
            ...
            # plantclef: Also enable KeepAlives on the accepted connection
    ...
    def run_worker(self):
        ...
        while True:
            try:
                ...
                # plantclef: Enable KeepAlives on the client side
    ...
    def execute_phase(self, phase):
        ...
        env = os.environ.copy()
        env["CLUSTER_NODE_RANK"] = str(self.rank)
        env["CLUSTER_MASTER_IP"] = self.master_ip
        env["PYTHONPATH"] = os.getcwd()
        
        launch_cmd = ["bash", "launch.sh", phase, self.role]
...
def main():
    parser = argparse.ArgumentParser(description="PLANTCLEF Unified Zero-Config Orchestrator")
