...
class OracleCLI:
    def __init__(self):
        self.parser = argparse.ArgumentParser(description="ORACLE: PlantCLEF 2026 Orchestrator")
        ...
        infer_parser.add_argument("role_pos", nargs="?",
                                  ...
                                  metavar="ROLE",
                                  help="Role shorthand positional (e.g. ./oracle.py infer --ensemble sprint)")
    ...
    def _handle_cache(self, args, unknown):
        ...
        print(f"[ORACLE] Teacher cache  role={args.role}  "
              f"res={args.res}  batch={args.batch}  out={out_dir}")
        env = os.environ.copy()
        if getattr(args, "rank", None) is not None:
            env["ORACLE_NODE_RANK"] = str(args.rank)
        env["ORACLE_SEED"] = str(args.seed)
        env["ORACLE_MODE"] = getattr(args, "mode", "auto")
        env["OMP_NUM_THREADS"] = "32"

        cmd = [
            "./src/setup/launch_oracle.sh", "cache", args.role,
    ...
    def _handle_dashboard(self, args):
        ...
        print(f"[ORACLE] Activating Neon Command Center on http://localhost:{port}")

        # 1. Start the Go API in the background
        print("[ORACLE] Starting backend API...")
        cwd = "orchestrator"
        binary = "./oracle_control"
        env = os.environ.copy()
        env["ORACLE_API_PORT"] = "8081" # Keep API on a separate port
        ...
        try:
            ...
            print(f"[ORACLE] Serving dashboard files from dashboard/dist...")
            
            # Use absolute path to ensure Python server finds the files
            dist_path = os.path.abspath("dashboard/dist")
    ...
    def _handle_train(self, args, unknown):
        ...
        run_name = f"oracle_s{args.seed}" if phase != "p1" else "phase1_extract"
        print(f"[ORACLE] {phase} ({args.role})  seed={args.seed}  config={cfg}")
        ...
        env = os.environ.copy()
        env["ORACLE_SEED"] = str(args.seed)
        env["ORACLE_NAME"] = f"oracle_s{args.seed}"
        env["ORACLE_MODE"] = getattr(args, "mode", "auto")
        ...
        # NODE_RANK, device count, ORACLE_MODE). Explicit --rank still
        ...
        if getattr(args, "cluster", None):
            ...
            print(f"[ORACLE] Cluster: {task.manifest.name if task.manifest else '-'}  "
                  f"Host: {task.host.id if task.host else '-'}  "
                  f"Role: {args.role}  "
                  f"Devices: {task.host.device_count if task.host else '-'}  "
                  f"Mode: {env.get('ORACLE_MODE')}")

        # All training phases (p1, p2a, p2b) now go through launch_oracle.sh
        # launch_oracle.sh handles the multi-seed looping for sprint roles internally.
        cmd = ["./src/setup/launch_oracle.sh", phase, args.role,
        ...
        try:
            ...
            print(f"[ORACLE] Phase {args.phase} failed. Check logs.")
            sys.exit(1)
    ...
    def _handle_infer(self, args, unknown):
        ...
        print(f"[ORACLE] Launching Inference ({role})  config={cfg}")
        
        if args.ensemble:
            # Full pipeline (SWA -> Opt -> Calibrate -> Ensemble)
            cmd = ["./src/setup/launch_oracle.sh", "pipeline", role]
        else:
            # Single-model inference
            cmd = ["./src/setup/launch_oracle.sh", "inf", role,
                   "--config", cfg]
