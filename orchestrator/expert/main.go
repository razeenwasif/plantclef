...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage: ./oracle_expert <phase> [extra_args...]")
		return
	}
	phase := os.Args[1]
	extraArgs := os.Args[2:]
...
func launchTraining(phase, role string, extraArgs []string) {
	...
	envCmd := fmt.Sprintf("source /workspace/pytorch_env/bin/activate && export ORACLE_NODE_RANK=%s && ", rankVar) +
			  "export TORCH_NCCL_ASYNC_ERROR_HANDLING=1 && " +
			  "export NCCL_DEBUG=WARN"

	launchPath := "/workspace/PlantCLEF2026/src/setup/launch_oracle.sh"
