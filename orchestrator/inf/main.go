...
func getMasterAddr() string {
	ip := getEnv("ORACLE_MASTER_IP", "")
	if ip == "" {
		content, err := os.ReadFile("/workspace/PlantCLEF2026/src/setup/.oracle_master_ip")
		if err == nil {
...
func getTotalWorkers() int {
	if val := os.Getenv("ORACLE_TOTAL_WORKERS"); val != "" {
		var res int
		fmt.Sscanf(val, "%d", &res)
		return res
	}
	if val := os.Getenv("ORACLE_NNODES"); val != "" {
		var res int
		fmt.Sscanf(val, "%d", &res)
		if res > 0 {
			return res - 1
...
func startTelemetryServer() {
	http.HandleFunc("/metrics", func(w http.ResponseWriter, r *http.Request) {
		...
		if _, err := os.Stat(logPath); err == nil {
			...
			if err != nil {
				fmt.Fprintf(w, "oracle_inf_status{state=\"offline\"} 0\n")
				return
			}

			fmt.Fprintf(w, "oracle_inf_status{state=\"running\"} 1\n")
			...
			for i := len(lines) - 1; i >= 0; i-- {
				...
				if strings.Contains(line, "img/s") {
					if idx := strings.Index(line, "img/s"); idx != -1 {
						...
						if len(parts) > 0 {
							fmt.Fprintf(w, "oracle_inf_speed_imgs %s\n", parts[len(parts)-1])
						}
					}
					if idx := strings.Index(line, "%|"); idx != -1 {
						progParts := strings.Fields(line[:idx])
						fmt.Fprintf(w, "oracle_inf_progress_percent %s\n", progParts[len(progParts)-1])
					}
					break
				}
			}
		} else {
			fmt.Fprintf(w, "oracle_inf_status{state=\"initializing\"} 0\n")
		}
	})
...
func main() {
	...
	fmt.Println("🛡️  ORACLE-ULTIMATE: Initializing Inference Orchestrator...")
	...
	for {
		if role == "master" {
			...
			rank := getEnv("ORACLE_NODE_RANK", "1")
			runWorker(rank)
		} else {
...
func launchInference(role string) {
	...
	rankVar := os.Getenv("ORACLE_NODE_RANK")
	if rankVar == "" {
		if role == "master" {
			rankVar = "0"
		} else {
	...
	envCmd := fmt.Sprintf("export ORACLE_NODE_RANK=%s && ", rankVar) +
		fmt.Sprintf("export ORACLE_MASTER_IP=%s && ", MasterAddr) +
	...
	// Using the unified oracle.py CLI instead of the legacy shell script
	cmdStr := fmt.Sprintf("%s && ./oracle.py infer --ensemble --role %s", envCmd, role)
