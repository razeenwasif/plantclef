...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage: ./oracle_control <phase|hub> [extra_args...]")
		return
	}
	phase := os.Args[1]
	extraArgs := os.Args[2:]
	...
	localIP := getLocalIP()
	masterFile := "/workspace/PlantCLEF2026/src/setup/.oracle_master_ip"
	masterIP := getEnv("ORACLE_MASTER_IP", "")
	isMaster := false
	...
	if phase == "hub" {
		isMaster = true
		masterIP = localIP
		fmt.Println("🛰️  ORACLE-HUB: Initializing Always-On Control Plane...")
	...
	if masterIP != "" {
		...
		os.Setenv("ORACLE_MASTER_IP", masterIP)
	}
	...
	if phase == "hub" {
		fmt.Printf("✅ ORACLE HUB ACTIVE. Dashboard: http://%s:8080\n", localIP)
	...
	// --- 3. Execution Loop ---
	if isMaster {
		fmt.Println("🛡️  ORACLE-ULTIMATE: [Master Hub] Initializing Control Plane...")
	} else {
		rank := os.Getenv("ORACLE_NODE_RANK")
		fmt.Printf("🛡️  ORACLE-ULTIMATE: [Worker %s] Initializing Compute Plane...\n", rank)
	}
...
func launchTraining(phase, role string, extraArgs []string) {
	...
	cmdStr := fmt.Sprintf("./src/setup/launch_oracle.sh %s %s %s", phase, role, argsStr)
