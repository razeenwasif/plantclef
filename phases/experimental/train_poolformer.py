"""PoolFormer trainer (MetaFormer family — attention replaced by average pooling).

Mirrors train_gfnet.py end-to-end; only the backbone construction differs.
PoolFormer is a strong fast-training baseline: no attention, no FFT, no SSM —
just depthwise pooling as the token mixer (Yu et al. 2022, "MetaFormer is
Actually What You Need").

Default backbone: poolformerv2_m48 (≈73 M params, ~ConvNeXt-B speed).
Other reasonable picks: poolformerv2_s24 (smaller, faster), poolformerv2_m36.
"""
import os
import math
import torch
...
# ── 1. Backbone ──────────────────────────────────────────────────────────────
def build_poolformer(model_name='poolformerv2_m48', num_classes=7806, resolution=224):
    print(f"[*] Building {model_name} at {resolution}px (num_classes={num_classes})...")
    model = timm.create_model(
        model_name,
        pretrained=True,
        num_classes=num_classes,
        # PoolFormer uses a 4-stage hierarchical layout; img_size is implicit
        # via the stem stride. Pass it for completeness when supported.
    )
    return model


# ── 2. Loaders (identical to train_gfnet.py) ──────────────────────────────────
...
def build_wds_loader(shards_pattern, batch_size, num_workers, resolution, is_train=True):
    ...
    # Heavy augmentation on train; keep val deterministic.
    if is_train:
...
def train(args):
    ...
    print(f"[*] Initializing PoolFormer Training on {device} | Epochs: {args.epochs}")
    ...
    model = build_poolformer(model_name=args.model, num_classes=args.num_classes, resolution=args.resolution).to(device)
    ...
    if args.compile:
        # PoolFormer is plain conv/pool — max-autotune is safe and fastest.
    ...
    out_dir = "models/poolformer"
    os.makedirs(out_dir, exist_ok=True)
    latest_ckpt_path = f"{out_dir}/latest_poolformer.pt"
    best_ckpt_path   = f"{out_dir}/best_poolformer.pt"
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # EMA model produces stronger, less-noisy val numbers — use it when available.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model",       type=str, default="poolformerv2_m48",
                        help="timm model name. Try poolformerv2_s24 (small/fast) or poolformerv2_m36.")
    ...
    # PoolFormer is fully pretrained on ImageNet — 5e-4 is the standard fine-tuning LR.
    # Bumping higher tends to wreck the pretrained backbone.
