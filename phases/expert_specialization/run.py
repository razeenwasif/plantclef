    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader):
            if i >= cfg.max_val_batches:
                break
            # ORACLE: Explicit device and dtype placement
            ...
            # ORACLE: One-time debug log to verify label alignment
...
def _train(cfg: P2bTeacherConfig, seed: int = 42) -> None:
    ...
    # ORACLE: Compile FIRST to allow standard DeepSpeed/Dynamo harmony
    ...
    # ORACLE: Calculate max epoch. If extra_epochs was provided via CLI, 
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        if res != current_res:
            ...
            # ORACLE: Synchronize compute engine with dynamic loader
        ...
        for i, data in enumerate(pbar):
            ...
            if model_engine.is_gradient_accumulation_boundary():
                ...
                # ORACLE: Periodic progress checkpoint (every 1000 GA steps)
...
def main() -> None:
    ...
    if args.epochs is not None:
        # ORACLE: Treat CLI --epochs as "additional epochs to run"
