...
------------
  ...
  grid_NxN         : NxN grid  (N=2,3,4,5 → 4,9,16,25 tiles)
  ...
  multiscale_dense : whole + grids from --scales + optional sliding window
...
def _tile_image_with_info(
    ...
    elif mode in ("grid_2x2", "grid_3x3", "grid_4x4", "grid_5x5"):
        n = int(mode.split("_")[1].split("x")[0])
        tiles = _tiles_grid(image, n, overlap_ratio)
...
def save_tile_preview(
    ...
    mode             : tiling mode string
    output_path      : where to write the PNG
    max_preview_tiles: cap for preview to avoid illegible images
    """
    pairs = _tile_image_with_info(
        image, mode,
        overlap_ratio=overlap_ratio,
...
def encode_tiles(
    ...
    Parameters
    ----------
    backbone_encode_fn : callable (B, 3, H, W) → (B, D)
    preprocess         : callable PIL Image → (3, H, W) tensor
    ...
    Returns
    -------
    (n_tiles, embed_dim) float32 CPU tensor; NOT L2-normalised.
    """
    if not tiles:
...
def classify_tiles(
    ...
    """
    Apply the linear classification head to tile features.

    Returns (n_tiles, num_classes) logits on CPU.
    """
    head.eval()
...
def infer_image(
    ...
    """
    Full tiled inference pipeline for a single image.

    Returns (num_classes,) image-level logits on CPU.
    """
    tiles = tile_image(
        image,
        mode=tile_mode,
