"""
Phase A — DINOv3-L full fine-tune on 1.4M single-plant PC24 images.

Runs on 1× GPU (``python train_phase_a.py ...``) or N× GPUs via DDP
(``torchrun --nproc_per_node=N train_phase_a.py ...``).

Recipe (PlantNet-style)
-----------------------
  * Warmup: freeze backbone, train head only for ``--warmup-epochs``.
  * Main:   unfreeze backbone, two LR groups (backbone smaller, head bigger),
            OneCycleLR, AdamW, bf16 autocast.
  * Loss:   Cross-entropy with logit adjustment ``log(class_freq/total)``
            added to the logits during training only.
  * Aug:    RandomResizedCrop + HFlip + ColorJitter + RandomErasing.

Output (rank 0 only)
--------------------
  ``{output-dir}/phase_a_ep{epoch}.pth``  — model + head + meta per epoch
  ``{output-dir}/phase_a_last.pth``       — rolling latest
  ``{output-dir}/phase_a_best.pth``       — best-by-top1-val

Launch examples
---------------
  # Single 5090:
  python3 train_phase_a.py --batch-size 48 --accum 2 ...

  # 2× 5090 DDP:
  torchrun --nproc_per_node=2 train_phase_a.py --batch-size 48 --accum 1 ...

  # 3× 5090 DDP:
  torchrun --nproc_per_node=3 train_phase_a.py --batch-size 48 --accum 1 ...

With DDP the effective batch is ``batch_size × accum × world_size``.
"""
...
from model import DINOv3SinglePlantClassifier, build_default_transform, build_train_transform
from single_plant_dataset import SinglePlantDataset, split_indices

# Reuse 004 for species_ids loading (canonical 7806-wide ordering).
...
def setup_distributed() -> tuple[int, int, int, bool]:
    ...
    When launched via ``torchrun`` the env vars RANK / WORLD_SIZE / LOCAL_RANK
    are set and NCCL init_process_group succeeds. When launched as plain
    ``python`` the returns are ``(0, 1, 0, False)``.
    """
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        dist.init_process_group(backend="nccl", timeout=__import__("datetime").timedelta(minutes=30))
        rank = dist.get_rank()
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--img-size", type=int, default=224,
                   help="Must be a multiple of 16 for DINOv3 patch-16.")
    # Training
    ...
    p.add_argument("--num-workers", type=int, default=12,
                   help="Per-GPU dataloader workers.")
...
def validate(model, loader, device: str, bf16: bool, is_dist: bool
    ...
    if is_dist:
        buf = torch.tensor([loss_sum, top1, top5, total], device=device, dtype=torch.float64)
...
def main() -> None:
    ...
    if args.img_size % 16 != 0:
        sys.exit("img-size must be a multiple of 16 for DINOv3 patch-16.")

    device = f"cuda:{local_rank}" if torch.cuda.is_available() else "cpu"
    species_ids = load_species_ids(args.species_csv)
    n_classes = len(species_ids)
    if is_main:
        logger.info(f"World size: {world} | device: {device} | N classes: {n_classes}")

    # Model ------------------------------------------------------------------
    model = DINOv3SinglePlantClassifier(n_classes=n_classes).to(device)
    ...
    full_train = SinglePlantDataset(args.train_csv, args.images_root, species_ids, transform=train_tf)
    full_val = SinglePlantDataset(args.train_csv, args.images_root, species_ids, transform=val_tf)
    ...
    log_prior = torch.log(freqs.clamp_min(1.0) / total).to(device=device, dtype=torch.float32)
    if is_main:
        logger.info(
    ...
    train_loader = DataLoader(
        train_ds, batch_size=args.batch_size, shuffle=train_shuffle, sampler=train_sampler,
    ...
    if args.warmup_epochs > 0 and args.resume is None:
        ...
        if is_dist:
            # DDP doesn't like buckets containing all-frozen params; use
            # find_unused_parameters during the warmup so backward still flows.
            m = DDP(model if not isinstance(model, DDP) else model.module,
                    device_ids=[local_rank], find_unused_parameters=True)
        else:
            m = model

        head_params = list(inner(m).head_parameters())
        optim = torch.optim.AdamW(head_params, lr=args.lr_head,
            ...
            for step, (imgs, targets) in enumerate(train_loader):
                ...
                if is_main and step % args.log_every == 0:
                    logger.info(
                        ...
                        f"loss={loss.item()*args.accum:.3f} lr={optim.param_groups[0]['lr']:.2e}"
                    )
            if is_main:
                logger.info(f"warmup epoch {epoch} done in {(time.time()-t0)/60:.1f}m, "
                            f"avg loss={running/max(n_micro,1):.3f}")
        # Tear down the warmup DDP wrapper (with find_unused_parameters)
        # so the main loop can build a fresh one without that flag.
        model = inner(m)

    # ------- Main: unfreeze backbone, DDP-wrap ------------------------------
    inner(model).unfreeze_backbone()
    if is_dist:
        model = DDP(model if not isinstance(model, DDP) else model.module,
    ...
    if is_main:
        ...
        logger.info(f"Trainable: backbone {n_bb/1e6:.1f}M, head {n_hd/1e6:.1f}M  "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Reduce training-loss sum across ranks for a clean per-epoch number.
        if is_dist:
            reduce_buf = torch.tensor([running, n_micro], device=device, dtype=torch.float64)
            dist.all_reduce(reduce_buf, op=dist.ReduceOp.SUM)
            running, n_micro = reduce_buf.tolist()
        avg_loss = running / max(n_micro, 1)

        val_loss, val_top1, val_top5 = validate(model, val_loader, device,
                                                bf16=args.bf16, is_dist=is_dist)
        if is_main:
            logger.info(
                f"epoch {epoch}/{args.epochs} | train_loss={avg_loss:.3f} "
                f"val_loss={val_loss:.3f} top1={val_top1:.4f} top5={val_top5:.4f} "
            ...
            if val_top1 > best_top1:
                ...
                logger.info(f"  new best val_top1={val_top1:.4f} -> {best_path.name}")

        # Broadcast best_top1 so every rank stays in sync (not strictly needed
        # but cheap, and avoids rank-0 drift on long runs).
        if is_dist:
