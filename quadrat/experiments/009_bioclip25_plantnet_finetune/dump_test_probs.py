"""
BioCLIP-2.5 fine-tuned tile-inference dump — produces a per-quadrat,
per-species probability matrix in the same npz schema 008 emits, so the
existing fusion / submission tooling works unchanged.

Differences vs ``008/dump_test_probs_phase_a.py``:
  * Loads ``BioCLIP25SinglePlantClassifier`` instead of the DINOv3 head.
  * Tile sizes must be multiples of 14 (ViT-H/14), not 16.
  * Uses BioCLIP normalization stats baked into ``build_default_transform``.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--checkpoint", required=True, help="phase_a_best.pth from 009 training")
    p.add_argument("--images-root", required=True)
    p.add_argument("--output", required=True)
    p.add_argument("--tile-sizes", type=int, nargs="+", default=[224, 336],
                   help="Multiples of 14. Default mirrors 008's 2-scale sweep "
                        "(224 = ViT-H/14 native, 336 = 24 patches).")
    ...
    p.add_argument("--whole-image", action="store_true",
                   help="Forward the whole image resized to the smallest tile size in addition to tiles.")
    return p.parse_args()


def load_bioclip25(ckpt_path: str | Path, device: str) -> tuple[BioCLIP25SinglePlantClassifier, list[str]]:
    ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
    species_ids: list[str] = ckpt["species_ids"]
    n_classes = int(ckpt.get("n_classes", len(species_ids)))
    # pretrained=True still pulls weights from HF hub for the visual tower
    # init; we then overwrite with the fine-tuned state. Loading pretrained
    # is necessary because open_clip's create_model_and_transforms always
    # requires the architecture config from the hub.
...
def main() -> None:
    ...
    for ts in tile_sizes:
        if ts % 14 != 0:
            sys.exit(f"tile size {ts} must be a multiple of 14 for BioCLIP-2.5 ViT-H/14.")
