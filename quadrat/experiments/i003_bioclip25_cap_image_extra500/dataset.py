...
# Be tolerant of partially-truncated JPEGs at runtime.  Bad files are still
# caught and skipped via __getitem__'s exception handler; this just lets PIL
# decode whatever bytes did arrive instead of bailing on the last 16 bytes.
ImageFile.LOAD_TRUNCATED_IMAGES = True
# Raise the decompression-bomb threshold (warnings/errors at ~89M pixels by
# default) since legitimate plant photos can be very high resolution.
...
DEFAULT_TRAIN_META_CSV = (
    ...
    "i003_bioclip25_cap_image_extra500/data/"
    "combined_old_extra_max500_train_manifest.csv"
)
...
class MultiTaskDataset(Dataset):
    ...
    _MAX_LOAD_RETRIES = 8
    _bad_path_log_count = 0
    _BAD_PATH_LOG_LIMIT = 50

    def __getitem__(self, idx: int):
        # If a file is truncated / not-an-image / decompression-bomb / I/O error,
        # advance to the next index in the same dataset.  Bounded retry avoids
        # pathological infinite recursion; on exhaustion the underlying error
        # propagates so the failure is visible.
        original_idx = idx
        for attempt in range(self._MAX_LOAD_RETRIES):
            ...
            try:
                ...
                if MultiTaskDataset._bad_path_log_count < MultiTaskDataset._BAD_PATH_LOG_LIMIT:
                    logger.warning(
                        f"Skipping bad image (attempt {attempt+1}/{self._MAX_LOAD_RETRIES}): "
                        f"{path}  [{type(e).__name__}: {e}]"
                    )
                    MultiTaskDataset._bad_path_log_count += 1
                idx = (idx + 1) % len(self.df)
                if idx == original_idx:
                    break
        raise RuntimeError(
            f"Failed to load any valid image after {self._MAX_LOAD_RETRIES} retries "
            f"starting from idx={original_idx}"
        )
