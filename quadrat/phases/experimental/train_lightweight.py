...
# os.environ["HF_HUB_OFFLINE"] = "1" # plantclef: Commented out to allow dynamic downloads if needed
...
def train_lightweight():
    parser = argparse.ArgumentParser(description="PLANTCLEF: Lightweight Model Training")
    ...
    # plantclef: Explicitly initialize CUDA context for DALI
    ...
    for epoch in range(args.epochs):
        ...
        if val_loader:
            ...
            val_total = 0
            with torch.no_grad():
                # plantclef: Autocast required for TransformerEngine layers in eval mode
                with torch.amp.autocast('cuda', dtype=torch.bfloat16):
