...
class InferenceEnsemble(nn.Module):
    ...
    def __init__(self, num_classes: int = 7806, input_res: int = 512, gating_dim: int = 3,
        ...
        # plantclef: Normalization Buffers.
    ...
    def from_checkpoint(
        ...
        # plantclef: Peek at checkpoint to detect architecture variants (e.g. FP8 gating)
        gating_dim = 3
    ...
    def forward(self, x: torch.Tensor, return_features: bool = False):
        ...
        # plantclef: Per-backbone normalization (BioCLIP uses CLIP stats, others use ImageNet).
...
class LegacyStandaloneModel(nn.Module):
    ...
    def __init__(self, checkpoint_path: str, resolution: int, device: str = "cuda"):
        ...
        # plantclef: Normalization Buffers (ImageNet defaults)
        self.register_buffer("mean", torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1))
        self.register_buffer("std",  torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1))
        ...
        if "species_head.weight" in sd: 
            ...
            # plantclef: Support for lightweight specialists (classifier.0=LN, classifier.2=Linear)
        ...
        if is_bioclip and not is_lightweight:
            ...
            # plantclef: Handle lightweight specialists trained with timm backbones
            from src.models.vit_backbone import PlantViTBackbone
        ...
        # 5. Final Head Init
        # plantclef: Adaptive Head Dimensioning
