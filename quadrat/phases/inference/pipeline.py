...
def _retinex_normalize(arr: np.ndarray, sigmas=(15.0, 80.0, 250.0)) -> np.ndarray:
    """PyTorch FFT-based Multi-Scale Retinex on GPU.
    Lightning-fast replacement for scipy.ndimage.gaussian_filter.
    arr is HxWx3 float32 in [0,1]."""
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    log_arr = np.log1p(arr * 255.0)
    t_log = torch.from_numpy(log_arr).permute(2, 0, 1).unsqueeze(0).to(device) # [1, 3, H, W]
    
    _, _, H, W = t_log.shape
    msr = torch.zeros_like(t_log)
    
    # Pre-compute frequency grids for FFT blur
    fy = torch.fft.fftfreq(H, d=1.0).view(-1, 1).to(device)
    fx = torch.fft.fftfreq(W, d=1.0).view(1, -1).to(device)
    freq_sq = (fx**2 + fy**2)
    
    t_log_fft = torch.fft.fft2(t_log)
    
    for s in sigmas:
        # Gaussian kernel in frequency domain
        kernel_f = torch.exp(-2.0 * (math.pi * s)**2 * freq_sq)
        blurred = torch.fft.ifft2(t_log_fft * kernel_f).real
        msr += (t_log - blurred)
    ...
    res = ((msr - lo) / (hi - lo)).squeeze(0).permute(1, 2, 0).cpu().numpy()
...
def _iter_test_images(cfg: InferenceConfig) -> Iterator[Tuple[str, str, int]]:
    if os.path.exists(cfg.test_csv):
        ...
        with open(cfg.test_csv) as f:
            ...
            for row in reader:
                ...
                # SOTA: Extract thermodynamic phenology month
                month = 7 # Default peak summer
                ...
                if os.path.exists(path):
                    yield img_id, path, month
    else:
        for p in sorted(Path(cfg.img_dir).rglob("*")):
            if p.suffix.lower() in (".jpg", ".jpeg", ".png", ".webp"):
                yield p.stem, str(p), 7


# ── Pauli Exclusion (Tile-Level NMS) ─────────────────────────────────────────

def _pauli_exclusion_nms(tile_logits: torch.Tensor, confidence_threshold: float = 0.85, decay_rate: float = 0.5) -> torch.Tensor:
    """
    Quantum-inspired spatial constraint:
    If a tile is strongly 'claimed' by a species (> confidence_threshold), 
    exponentially decay the logits of competing species for that specific physical tile 
    before aggregation.
    """
    probs = torch.sigmoid(tile_logits) # [T, C]
    
    # Find tiles where the dominant species is highly confident
    max_probs, dominant_classes = probs.max(dim=1)
    strong_claims = max_probs > confidence_threshold
    
    if not strong_claims.any():
        return tile_logits
        
    adjusted_logits = tile_logits.clone()
    
    # For each tile with a strong claim
    for t_idx in torch.where(strong_claims)[0]:
        dom_class = dominant_classes[t_idx]
        dom_prob  = max_probs[t_idx]
        
        # Calculate penalty based on the dominant confidence
        # e.g., if prob is 0.99, penalty is severe. If 0.86, penalty is mild.
        penalty_factor = 1.0 - (decay_rate * (dom_prob - confidence_threshold) / (1.0 - confidence_threshold))
        
        # Apply penalty to all classes in this tile
        adjusted_logits[t_idx] = adjusted_logits[t_idx] * penalty_factor
        # Restore the dominant class's logit so it remains unaffected
        adjusted_logits[t_idx, dom_class] = tile_logits[t_idx, dom_class]
        
    return adjusted_logits

# ── Aggregation (within-image, across tiles) ─────────────────────────────────

def _aggregate(tile_logits: torch.Tensor, method: str, weights: torch.Tensor | None = None) -> torch.Tensor:
    # SOTA: Apply Pauli Exclusion NMS before aggregation
    # tile_logits = _pauli_exclusion_nms(tile_logits) # Disabled: Dropped score
    ...
    if method == "mean":
        ...
        # 1. Negative Entropy Weighting (Bayesian Certainty)
        probs = torch.softmax(tile_logits, dim=1)
        entropy = -torch.sum(probs * torch.log(probs + 1e-12), dim=1)
        bayes_weights = torch.exp(-entropy)
        
        # 2. Combine with Physical Vegetation Weights (ExG)
        if weights is not None:
            # Align weights to same device as logits
            weights = weights.to(tile_logits.device)
            final_weights = bayes_weights * weights
        else:
            final_weights = bayes_weights
            
        # 3. Normalized Weighted Sum
        final_weights = final_weights / (final_weights.sum() + 1e-12)
        return (tile_logits * final_weights.unsqueeze(1)).sum(0)
    else:                                        # conf_weighted
        probs   = torch.softmax(tile_logits, dim=1)
...
def _load_taxon_filter(cfg: InferenceConfig):
    ...
    try:
        ...
        # plantclef: Align to model's num_classes (e.g. 7806)
...
def _load_pagerank_matrix(cfg: InferenceConfig):
    """Builds a sparse transition matrix for PageRank (Random Walk with Restart)
    based on the ecological co-occurrence graph."""
    ...
    try:
        ...
        N = cfg.num_classes
        # Using a dense matrix as 7806x7806 floats is only ~240MB, highly efficient on GPU
        transitions = torch.zeros((N, N), dtype=torch.float32)
        for i in range(min(len(adj), N)):
            neighbors = [n for n in adj[i] if n < N]
            if neighbors:
                deg = len(neighbors)
                for n in neighbors:
                    transitions[n, i] = 1.0 / deg
            else:
                transitions[i, i] = 1.0 # self-loop
                
        # Self-loops for missing nodes
        for i in range(len(adj), N):
            transitions[i, i] = 1.0
            
        print(f"[Inference] PageRank Ecological Transition Matrix loaded.")
        return transitions
    except Exception as e:
        print(f"[Inference] WARN: PageRank init failed ({e}); skipping.")
        return None
...
def _load_faiss_engine(cfg: InferenceConfig):
    ...
    try:
        ...
        idx.add(protos_np)
        # plantclef: Explicitly run FAISS on CPU to avoid kernel image errors on B200 (Blackwell)
        placement = "CPU"
...
def _load_ising_solver(cfg: InferenceConfig):
    if not cfg.use_ising_model:
        return None
    try:
        from src.inference.ising_model import SpinGlassSolver
        # Prefer the offline allelopathy matrix when present — far richer J than the
        # sparse graph-edge coupling. The solver auto-falls-back to the graph if the
        # path doesn't exist.
        j_path = "models/priors/allelopathy_matrix.pt"
        if os.path.exists(j_path):
            ising = SpinGlassSolver(num_classes=cfg.num_classes, j_matrix_path=j_path)
        else:
            ising = SpinGlassSolver("data/taxonomic_graph.json", num_classes=cfg.num_classes)
        print(f"[Inference] Spin Glass (Ising Model) solver loaded.")
        return ising
    except Exception as e:
        print(f"[Inference] WARN: Ising solver init failed ({e}); skipping.")
        return None

def _load_phenology_mask(cfg: InferenceConfig):
    if getattr(cfg, "use_phenology", False):
        try:
            from src.inference.phenology import ThermodynamicPhenology
            pheno = ThermodynamicPhenology(num_classes=cfg.num_classes)
            print(f"[Inference] Thermodynamic Phenology mask loaded.")
            return pheno
        except Exception as e:
            print(f"[Inference] WARN: Phenology init failed ({e}); skipping.")
    return None

def _load_geochem_masker(cfg: InferenceConfig):
    """DEPRECATED: simulated GeochemMasker (used a hard-coded local_ph=7.0).
    Kept only for backwards compatibility; new pipeline uses the offline EIVE-derived
    matrix loaded by `_load_geochem_matrix`.
    """
    return None


def _load_geochem_matrix(cfg: InferenceConfig):
    """Loads the offline EIVE-derived geochem repulsion matrix.

    Built by tools/build_environmental_priors.py from data/eive_compatibility.npy.
    Sparse: ~15.6k non-zero entries out of 60M, with strong negative values for
    pairs that disagree on light/temp/moisture/soil-pH/nutrients/salinity.
    """
    if not getattr(cfg, "use_geochem", False):
        return None
    matrix_path = "models/priors/geochem_matrix.pt"
    if os.path.exists(matrix_path):
        try:
            matrix = torch.load(matrix_path, map_location="cpu", weights_only=True)
            print(f"[Inference] Offline EIVE Geochem matrix loaded.")
            return matrix
        except Exception as e:
            print(f"[Inference] WARN: Failed to load geochem matrix ({e}).")
    print(f"[Inference] WARN: Geochem matrix not found at {matrix_path}; Geochem disabled.")
    return None

def _load_allelopathy_matrix(cfg: InferenceConfig):
    """Builds or loads the chemical antagonism matrix for Allelopathic Repulsion."""
    matrix_path = "models/priors/allelopathy_matrix.pt"
    if os.path.exists(matrix_path):
        try:
            matrix = torch.load(matrix_path, map_location="cpu", weights_only=True)
            print(f"[Inference] Offline Allelopathic Repulsion matrix loaded.")
            return matrix
        except Exception as e:
            print(f"[Inference] WARN: Failed to load allelopathy matrix ({e}).")
    
    print(f"[Inference] WARN: Allelopathic matrix not found at {matrix_path}; Repulsion disabled.")
    return None


def _load_logit_priors(cfg: InferenceConfig, device: torch.device) -> torch.Tensor | None:
    if cfg.logit_adj_tau <= 0.0:
        return None
    counts_path = "/workspace/plantclef/processed/species_train_counts.csv"
    mapping_path = cfg.species_mapping
    if not os.path.exists(counts_path) or not os.path.exists(mapping_path):
        print(f"[Inference] WARN: Logit adjustment files missing; tau disabled.")
        return None
    try:
        import pandas as pd
        ...
        print(f"[Inference] Logit Adjustment priors cached on {device} (tau={cfg.logit_adj_tau}).")
        return priors
    except Exception as e:
        print(f"[Inference] WARN: Logit Adjustment init failed ({e}); skipping.")
        return None
...
def _postprocess(logits: torch.Tensor, cfg: InferenceConfig, species_ids: List[str],
                 ...
                 fw_solver=None,
                 ising_solver=None,
                 pagerank_matrix: torch.Tensor | None = None,
                 phenology_mask=None,
                 geochem_masker=None,                              # legacy (always None now)
                 geochem_matrix: torch.Tensor | None = None,       # EIVE-derived [C, C]
                 allelopathy_matrix: torch.Tensor | None = None,
                 logit_priors: torch.Tensor | None = None) -> List[str]:
    ...
    stats = {"pav": 0, "ac3": 0, "fw": 0, "f1": 0.0, "phen": 0, "geochem": 0.0, "allelo": 0.0}

    # plantclef: FORCE GLOBAL ALIGNMENT
    if logits.shape[0] > cfg.num_classes:
        logits = logits[:cfg.num_classes]

    # ── Thermodynamic Phenology (Atmospheric Physics) ──────────────────────
    if phenology_mask is not None:
        try:
            before = logits.clone()
            # plantclef: Support beta from config (default 1.0)
            beta = getattr(cfg, "phenology_beta", 1.0)
            logits = phenology_mask.apply_thermal_penalty(logits, month, beta=beta)
            # Any non-trivial penalty counts (was 0.1 — too coarse).
            stats["phen"] = int(((before - logits).abs() > 1e-3).sum().item())
        except Exception as e:
            print(f"[Inference] WARN: Phenology mask failed ({e}); skipping.")

    # ── Geochem & Edaphic Repulsion (EIVE Mean-Field) ──────────────────────
    # Sparse [C, C] EIVE-derived matrix built by tools/build_environmental_priors.py.
    # Non-zero entries (~15.6k out of 60M) penalize pairs that disagree on
    # light/temp/moisture/soil-pH/nutrients/salinity. Centered like allelopathy
    # so the constant offset doesn't shift sigmoid calibration.
    if geochem_matrix is not None:
        try:
            # plantclef: Force float() to prevent "expected Float but found Double" error
            p_now = torch.sigmoid(logits).float()
            geo_penalty = geochem_matrix @ p_now  # [C]
            geo_penalty = geo_penalty - geo_penalty.mean()
            logits = logits + geo_penalty
            stats["geochem"] = float(geo_penalty.abs().mean().item())
        except Exception as e:
            print(f"[Inference] WARN: Geochem penalty failed ({e}); skipping.")

    # ── Allelopathic Repulsion (Mean-Field Penalty) ────────────────────────
    # Mean-field application of the offline [C, C] repulsion matrix:
    #   penalty[i] = Σ_j J[i, j] * σ(logit_j)
    # J is ≤ 0 everywhere (0 inside the 2-hop safe zone, -0.1 outside), so
    # species i is pushed down whenever topologically distant species j hold
    # significant probability mass. Applied AFTER phenology/geochem (so those
    # priors shape the σ() estimate that drives the repulsion).
    if allelopathy_matrix is not None:
        try:
            # plantclef: Force float() to prevent "expected Float but found Double" error
            p_now = torch.sigmoid(logits).float()
            penalty = allelopathy_matrix @ p_now  # [C]
            # CENTER the penalty: the dense -0.1 matrix produces a near-uniform shift of
            # -3 to -7 logits across all species, which crushes sigmoid calibration
            # (max prob 0.07 → 0.006). Only the *relative* repulsion between species
            # carries signal; the constant offset is noise. Centering preserves the
            # ranking signal while removing the calibration kill.
            penalty = penalty - penalty.mean()
            logits = logits + penalty
            stats["allelo"] = float(penalty.abs().mean().item())
        except Exception as e:
            print(f"[Inference] WARN: Allelopathy penalty failed ({e}); skipping.")

    # PLANTCLEF Day 1: Logit Adjustment
    if logit_priors is not None and cfg.logit_adj_tau > 0.0:
        logits = logits - cfg.logit_adj_tau * torch.log(logit_priors + 1e-9)
    ...
    # ── PageRank Ecological Diffusion (Random Walk with Restart) ───────────
    if pagerank_matrix is not None:
        pass # Disabled: Dropped score due to graph connectivity smoothing
        # try:
        #     device = probs.device
        #     M = pagerank_matrix.to(device)
        #     # v is the initial teleportation vector (model's raw confidence)
        #     v = probs.clone()
        #     v_norm = v / v.sum().clamp(min=1e-9)
        #     
        #     x = v_norm.clone()
        #     alpha = 0.85 # Damping factor
        #     
        #     # Sparse matmul diffusion
        #     for _ in range(5):
        #         x = (1 - alpha) * (M @ x) + alpha * v_norm
        #         
        #     # Blend the diffused probabilities back with the original
        #     # This boosts co-occurring flora while preserving original magnitude
        #     probs = alpha * probs + (1 - alpha) * (x * probs.sum())
        # except Exception as e:
        #     print(f"[Inference] WARN: PageRank Diffusion failed ({e}); skipping.")

    # ── Multi-Scale Thermodynamic Decoder (Ising Refiner) ──────────────────
    # 1. Macro-Scale: Frank-Wolfe rapidly carves out a bounding box of candidates
    # 2. Micro-Scale: Ising Model runs deep simulated annealing to find the ground state
    
    above = []
    if fw_solver is not None and ising_solver is not None:
        try:
            # Coarse Filter: Ask FW for 2x candidates so Ising has room to prune
            coarse_k = min(cfg.num_classes, cfg.fw_top_k * 2)
            sel_coarse = fw_solver.solve(logits.float(), max_iters=3, sparsity_k=coarse_k)
            candidate_indices = torch.nonzero(sel_coarse > 1e-3).squeeze(-1)
            
            if candidate_indices.numel() > 0:
                # Isolate the logits for just these candidates to speed up annealing
                # We artificially suppress all other species to -inf
                masked_probs = torch.full_like(probs, -float('inf'))
                masked_probs[candidate_indices] = probs[candidate_indices]
                
                # Deep Annealing on the restricted space
                sel_fine = ising_solver.solve(masked_probs, init_k=cfg.fw_top_k, max_iters=2000, cooling_rate=0.995)
                above = torch.nonzero(sel_fine > 0.5).squeeze(-1).tolist()
                stats["fw"] = len(above)
            else:
                above = [int(torch.argmax(probs))]
        except Exception as e:
            print(f"[Inference] WARN: Ising Refiner failed ({e}); falling back to standard FW.")
            above = []

    # Path A1: Frank-Wolfe Fallback (if Ising Refiner failed or ising disabled)
    if not above and fw_solver is not None:
        try:
    ...
    # Path A2: Spin Glass Fallback (if FW disabled but Ising enabled)
    if not above and fw_solver is None and ising_solver is not None:
        try:
            sel = ising_solver.solve(probs.float(), init_k=cfg.fw_top_k)
            above = torch.nonzero(sel > 0.5).squeeze(-1).tolist()
            if isinstance(above, int): above = [above]
        except Exception as e:
            print(f"[Inference] WARN: Ising solver failed on this image ({e}); falling back.")
            ising_solver = None

    if not above and fw_solver is None and ising_solver is None:
        ...
        # Path C: Brent's per-class thresholds
            ...
            if len(above) < cfg.min_predictions:
                tk = torch.topk(probs, k=cfg.top_k)
                above = [i for v, i in zip(tk.values.tolist(), tk.indices.tolist()) if v > 1e-9]
    ...
    # Safety net for common species: always include the top-2 by raw (post-AC3) probability.
    # FW + island-biogeography prior optimizes for diverse/sparse, which can prune
    # high-confidence common species in favor of rare ones. Forcing the top-2 raw picks
    # back in costs at most 2 false positives in the worst case but recovers commons
    # the model is confident about.
    #
    # IMPORTANT: filter by probability > 0. AC-3 zeros ~7800/7806 species, so on most
    # images probs has only a handful of non-zero entries. torch.topk on a near-zero
    # tensor returns lowest-indexed entries as tie-breakers, which historically
    # injected classes 0 and 1 (raw_ids 1355868/1355869) into ~50% of all predictions.
    if above:
        top2 = torch.topk(probs, k=min(2, probs.numel())).indices.tolist()
        seen = set(above)
        for i in top2:
            if probs[i].item() > 1e-9 and i not in seen:
                above.append(i); seen.add(i)
...
def _resolve_models(cfg: InferenceConfig, device: torch.device) -> List[Tuple[InferenceEnsemble, int]]:
    ...
    for e in present:
        # plantclef: Detect God-Tier / Legacy Standalone models
        is_legacy = any(x in e["path"] for x in ["best.pt", "plantnet_v2", "Exp008", "phase_b_best", "lightweight"])
        is_crt    = "crt" in e["path"].lower()
        is_multitask = any(x in e["path"] for x in ["i002", "010"]) or "multitask" in e["path"].lower()

        if is_crt:
            from .model import CRTInferenceEnsemble
            m = CRTInferenceEnsemble.from_crt_dir(
                crt_dir       = e["path"] if os.path.isdir(e["path"]) else os.path.dirname(e["path"]),
            ...
            from .model import BioCLIPMultiTaskEnsemble
            m = BioCLIPMultiTaskEnsemble(
...
def run_inference(cfg: InferenceConfig) -> None:
    ...
    ising_solver           = _load_ising_solver(cfg)
    sam_generator          = _load_sam_generator(cfg)
    pagerank_matrix        = _load_pagerank_matrix(cfg)
    phenology_mask         = _load_phenology_mask(cfg)
    geochem_masker         = _load_geochem_masker(cfg)        # legacy placeholder; returns None
    geochem_matrix         = _load_geochem_matrix(cfg)         # offline EIVE-based matrix
    allelopathy_matrix     = _load_allelopathy_matrix(cfg)
    # Park the [C, C] matrices on the inference device once. Otherwise we'd pay a
    # 244 MB H2D transfer per image inside _postprocess_predictions.
    if allelopathy_matrix is not None:
        allelopathy_matrix = allelopathy_matrix.to(device, non_blocking=True)
    if geochem_matrix is not None:
        geochem_matrix = geochem_matrix.to(device, non_blocking=True)
    faiss_index, _faiss_dim = _load_faiss_engine(cfg)
    logit_priors           = _load_logit_priors(cfg, device)

    # Agentic CV: LLM Arbiter
    llm_arbiter = None
    if getattr(cfg, "use_llm_arbiter", False):
        try:
            from src.inference.llm_arbiter import AgenticLLMArbiter
            llm_arbiter = AgenticLLMArbiter(
                use_gemma=cfg.use_gemma_tiebreaker, 
                use_nemotron=cfg.use_nemotron_veto,
                endpoint="http://localhost:11434/api/generate"
            )
            print("[Inference] Agentic CV LLM Arbiter initialized (VCoT active).")
        except Exception as e:
            print(f"[Inference] WARN: Failed to initialize LLM Arbiter ({e}).")

    # Self-Healing
    healing_engine = None
    if getattr(cfg, "use_async_healing", False):
        try:
            from src.inference.self_healing import AsyncHealingEngine
            healing_engine = AsyncHealingEngine(cfg.num_classes)
            print("[Inference] Async Self-Healing Engine initialized.")
        except Exception as e:
            print(f"[Inference] WARN: Failed to initialize Healing Engine ({e}).")
    ...
    # plantclef: Test-Time Training (TTT) adaptor. Only uses the first model for speed.
    ttt_adaptor = None
    ...
    def _infer_one_image(img: torch.Tensor, img_pil=None, img_id: str = "") -> torch.Tensor:
        ...
        veg_fractions = None
        if cfg.min_vegetation_frac > 0 or cfg.aggregation == "bayesian_veg":
            # Calculate fractions for ALL tiles
            fracs = []
            for t in tiles:
                mask = _exg_vegetation_mask(t.numpy())
                fracs.append(mask.mean())
            ...
            if cfg.min_vegetation_frac > 0:
                # Hard filter: only keep tiles above threshold
                tiles = [t for t, f in zip(tiles, fracs) if f >= cfg.min_vegetation_frac]
                if not tiles: # Fallback if all filtered
                    tiles = [_generate_tiles(img, cfg.tile_size, cfg.tile_overlap, [1.0])[0]]
                    veg_fractions = torch.tensor([1.0])
                else:
                    # Update fractions to match filtered tiles
                    veg_fractions = torch.tensor([f for f in fracs if f >= cfg.min_vegetation_frac])
            else:
                veg_fractions = torch.tensor(fracs)

        # --- SOTA: TTT Entropy Minimization ---
        ttt_loss = 0.0
        if ttt_adaptor is not None and len(tiles) > 0:
            # Adapt on a representative subset of tiles
            ttt_batch = torch.stack([
                F.interpolate(t.unsqueeze(0), size=(models[0][1], models[0][1]),
                              mode="bilinear", align_corners=False).squeeze(0)
                for t in tiles[:8]
            ])
            ttt_loss = ttt_adaptor.adapt(ttt_batch, image_id=img_id)
        ...
        for model, native_res in models:
            ...
            # plantclef: B200 Optimized Tile Batching
            all_logits_T = []
            all_feats_T  = []
            ...
            # plantclef: Slice to 7806 if model has padding (7808)
            aggregated = _aggregate(logits_T.to(device), cfg.aggregation, weights=veg_fractions)
        ...
        # FAISS retrieval boost — only fires for low-confidence or high-energy (OOD) predictions.
        if faiss_index is not None and all_query_features:
            probs_for_check = torch.sigmoid(agg_logits)
            
            # SOTA: Energy-Based OOD detection
            # Energy E(x) = -T * logsumexp(logits / T)
            # High energy = Out of Distribution (unseen species)
            energy = -1.0 * torch.logsumexp(agg_logits, dim=0).item()
            
            # Trigger if max prob is low OR energy is unusually high (OOD)
            is_low_conf = probs_for_check.max().item() < cfg.faiss_low_conf_threshold
            is_ood      = energy > 10.0 # Threshold calibrated for 7806-class logsumexp
            
            if is_low_conf or is_ood:
                if is_ood: print(f" [OOD] Energy detected: {energy:.2f} (triggering FAISS)")
    ...
    for img_id, img_path, month in my_images:
        try:
        ...
        original_logits = agg_logits.clone()
        if healing_engine is not None:
            agg_logits = healing_engine.forward(agg_logits)

        # Apply LLM Arbitration before mathematical thresholds
        if llm_arbiter is not None:
            # Prepare metadata for Nemotron
            # Assuming month is available, we could extend this with geo_ph, gdd if we loaded the CSV
            metadata = {"gdd": 1000, "ph": 7.0} # Placeholder values
            try:
                agg_logits = llm_arbiter.process_predictions(img_pil, agg_logits, species_ids, metadata)
            except Exception as e:
                print(f"[Inference] WARN: LLM Arbiter failed on {img_id} ({e})")

        preds, pp_stats = _postprocess(
            agg_logits, cfg, species_ids,
            month=month,
            per_class_thr=per_class_thr, q_hat=q_hat,
            pav_solver=pav_solver, taxon_filter=taxon_filter, fw_solver=fw_solver,
            ising_solver=ising_solver,
            pagerank_matrix=pagerank_matrix,
            phenology_mask=phenology_mask,
            geochem_masker=geochem_masker,
            geochem_matrix=geochem_matrix,
            allelopathy_matrix=allelopathy_matrix,
            logit_priors=logit_priors,
            return_stats=True
        )

        if healing_engine is not None:
            # Check if symbolic logic caused a top-1 change
            orig_top1 = torch.argmax(original_logits).item()
            new_top1_str = preds[0] if len(preds) > 0 else species_ids[orig_top1]
            try:
                new_top1 = species_ids.index(new_top1_str)
            except ValueError:
                new_top1 = orig_top1
            
            if orig_top1 != new_top1:
                # Symbolic Veto occurred! Request async heal
                target_dist = torch.zeros_like(original_logits)
                for p_str in preds:
                    try:
                        target_dist[species_ids.index(p_str)] = 1.0 / len(preds)
                    except ValueError:
                        pass
                healing_engine.request_heal(original_logits, target_dist)
        ...
        if pbar is not None:
            ...
            pbar.set_postfix(
                ...
                phen=pp_stats.get("phen", 0),
                geo=f"{pp_stats.get('geochem', 0.0):.2f}",
                allelo=f"{pp_stats.get('allelo', 0.0):.2f}",
                pav=pp_stats["pav"],
                ac3=pp_stats["ac3"],
                fw=pp_stats["fw"]
            )
    ...
    # plantclef: Gather all results across GPUs
    if cfg.world_size > 1:
        dist.barrier()
    ...
    if cfg.rank == 0:
        ...
        # plantclef: Automated Submission Validation
