...
class BioCLIP25MultiTask(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return self.forward_heads(feat)

    def forward_heads(self, feat: torch.Tensor):
        """
        Run only the head MLPs + classifiers against pre-computed backbone
        features. Used by the feature-cache training path
        (`train.py --feature-cache`), where the frozen backbone has already
        been evaluated offline and the cache holds (B, embed_dim) tensors.
        """
