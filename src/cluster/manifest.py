...
increment a rank counter" dance that ``launch_oracle.sh`` does today.
The launcher still works without a manifest (back-compat), but when
``--cluster cluster.yaml`` is passed to ``oracle.py``, the env vars it
...
* ``mode`` here is the cluster's preferred accelerator. Individual hosts
  *can* still override via ``ORACLE_MODE`` env, but most clusters are
...
class Manifest:
    ...
    def env_for(self, host: HostEntry) -> Dict[str, str]:
        ...
        env: Dict[str, str] = {
            ...
            "ORACLE_NNODES": str(len(self.hosts)),
            "ORACLE_MASTER_IP": self.master().hostname,
            "ORACLE_MASTER_PORT": str(self.master_port),
        }
        # Per-host accelerator capacity
        if host.type == "tpu":
            env["ORACLE_TPU_CORES"] = str(host.device_count)
        else:
            env["ORACLE_GPUS"] = str(host.device_count)
