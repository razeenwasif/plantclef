"""
TrainingTask — typed unit of work for ORACLE training/cache/inference.
...
``oracle.py`` builds a task from CLI args and hands it to ``launch_oracle.sh``
...
class TrainingTask:
    ...
    seed: int = 42
    name: Optional[str] = None              # ORACLE_NAME
    ...
    def to_env(self) -> Dict[str, str]:
        """Build the env dict launch_oracle.sh expects. Manifest-derived
        values (NNODES, MASTER_IP, NODE_RANK, GPUs/TPU cores, mode) win
        over the plain task fields when a manifest is supplied."""
        env: Dict[str, str] = {
            "ORACLE_SEED": str(self.seed),
            "ORACLE_NAME": self.name or f"oracle_s{self.seed}",
    ...
    def to_launcher_argv(self) -> List[str]:
        """Build the launch_oracle.sh argv (after the phase / role positional)."""
...
def build_task_from_args(args: Any, unknown: List[str]) -> TrainingTask:
    """Convert the argparse Namespace from oracle.py's `train` subparser
