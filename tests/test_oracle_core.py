...
from src.config import load_config, OracleConfig
...
def test_config_defaults():
    """Verify that OracleConfig initializes with correct default schema."""
    config = load_config()
...
@patch("oracle.subprocess.run")
def test_oracle_cli_dispatch(mock_run):
    """Verify that OracleCLI correctly dispatches shell commands."""
    from oracle import OracleCLI
    import sys
    
    cli = OracleCLI()
    # Mock sys.argv - Using the correct flag-based syntax
    with patch.object(sys, 'argv', ['oracle.py', 'train', '--phase', 'p2b', '--seed', '123']):
        cli.run()
    ...
    cmd = args[0]
    assert any("launch_oracle.sh" in part for part in cmd)
