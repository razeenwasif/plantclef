...
from model import load_checkpoint_model, BIOCLIP25_MODEL_NAME, resize_visual_pos_embed
...
def infer_image(
    ...
    With hflip_tta=True, each tile is also forwarded flipped horizontally and
    the flipped logits are appended as extra rows. softmax_mean aggregation
    then sees 2N views per image, equivalent to canonical hflip TTA.

    Returns
    -------
    tiles       : list of (TileInfo, PIL.Image)
    tile_logits : float32 CPU tensor of shape (N or 2N, num_species)
    """
    ...
    flipped_logits: list[torch.Tensor] = []
    for i in range(0, len(tile_tensors), batch_size):
        ...
        with amp_autocast(device, amp_enabled, amp_dtype):
            ...
            if hflip_tta:
                sp_logits_f, *_ = model(batch.flip(-1))
                flipped_logits.append(sp_logits_f.float().cpu())

    tile_logits = torch.cat(all_logits, dim=0)  # (num_tiles, num_species)
    if hflip_tta:
        tile_logits = torch.cat([tile_logits, torch.cat(flipped_logits, dim=0)], dim=0)
...
def build_logit_adjustment(
    ...
    """
    Per-class logit adjustment vector from training-set class frequencies.

    At inference: adjusted_logit[c] = raw_logit[c] - tau * log(prior[c])
    Subtracts log(prior) so rare classes (large-negative log) get boosted,
    common ones suppressed. Matches LogitAdjustmentLoss in src/training/losses.py.
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--hflip-tta", action="store_true",
                   help="Append horizontally-flipped tile views to logits before aggregation.")

    # Logit adjustment (long-tail calibration)
    p.add_argument("--logit-adj-tau", type=float, default=0.0, metavar="TAU",
                   help="Logit adjustment strength (0 disables). Subtracts tau*log(prior) from each class logit. Requires --metadata-csv.")
    p.add_argument("--metadata-csv", default=None,
                   help="Training metadata CSV with 'species_id' column (required when --logit-adj-tau > 0).")
...
def main() -> None:
    ...
    if args.img_size != 224:
        resize_visual_pos_embed(model, args.img_size, patch_size=14)
    model.eval()
